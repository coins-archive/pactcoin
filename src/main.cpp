...
#if defined(NDEBUG)
#error "PACT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in upact) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock) {
    //Check to see if the zPACT is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zPACT spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zPACT without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zPACT spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zPACT spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zPACT spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpact");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zPACT mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
int64_t GetBlockValue(int nHeight)
{
    if (nHeight == 0) {
        ...
        return 25 * COIN;
    } else if (nHeight >= 25000 && nHeight < 28000) {
        return 30 * COIN;
    } else if (nHeight >= 28000 && nHeight < 31000) {
        return 35 * COIN;
    } else if (nHeight >= 31000 && nHeight < 34000) {
        return 40 * COIN;
    } else if (nHeight >= 34000 && nHeight < 37000) {
        return 45 * COIN;
    } else if (nHeight >= 37000 && nHeight < 40000) {
        return 50 * COIN;
    } else if (nHeight >= 40000 && nHeight < 43000) {
        return 55 * COIN;
    } else if (nHeight >= 43000 && nHeight < 46000) {
        return 60 * COIN;
    } else if (nHeight >= 46000 && nHeight < 49000) {
        return 65 * COIN;
    } else if (nHeight >= 49000 && nHeight < 52000) {
        return 70 * COIN;
    } else if (nHeight >= 52000 && nHeight < 55000) {
        return 55 * COIN;
    } else if (nHeight >= 55000 && nHeight < 58000) {
        return 40 * COIN;
    } else if (nHeight >= 58000 && nHeight < 61000) {
        return 25 * COIN;
    } else {
        return 10 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    } else if (nHeight >= 225 && nHeight < 10000) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from PACT
...
{
    RenameThread("pact-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZPACTMinted()
{
...
void RecalculateZPACTSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zPACT supply
...
bool RecalculatePACTSupply(int nHeightStart)
{
...
bool UpdateZPACTSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    UpdateZPACTSupply(block, pindex);
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zPACTSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert PACT to zPACT
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zPACT spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zPACT serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zPACT mints and %d zPACT spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/PACT Core:0.1.0/" ||
            pfrom->cleanSubVer == "/PACT Core:0.3.0/" ||
            pfrom->cleanSubVer == "/PACT Core:0.3.1/") {
            LOCK(cs_main);
        ...
        // PACT: We use certain sporks during IBD, so check to see if they are
