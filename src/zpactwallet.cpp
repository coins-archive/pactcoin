...
CzPACTWallet::CzPACTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpact seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPACTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPACTSeed_deprecated()) {
                    LogPrintf("%s: Updated zPACT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpact seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpact wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPACTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPACTCount(nCountLastUsed);
    else if (!walletdb.ReadZPACTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPACTWallet::Lock()
{
    seedMaster = 0;
}

void CzPACTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPACTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPACT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPACTWallet::LoadMintPoolFromDB()
{
...
void CzPACTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPACTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPACTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpactTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPACTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPACT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpactTracker which also adds to database
    pwalletMain->zpactTracker->Add(dMint, true);
...
void CzPACTWallet::SeedToZPACT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPACTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPACTWallet::GenerateDeterministicZPACT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPACTWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPACTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
