...
#ifndef PACT_ZPACTWALLET_H
#define PACT_ZPACTWALLET_H
...
public:
    CzPACTWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZPACT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZPACT(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //PACT_ZPACTWALLET_H
