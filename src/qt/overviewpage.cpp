...
// Copyright (c) 2017-2019 The PACT Developers
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sPACTPercentage, QString& szPACTPercentage)
{
    int nPrecision = 2;
    ...
    szPACTPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sPACTPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount pactAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = pactAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // PACT labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, pactAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelPACTPercent->setText(sPercentage);
    ui->labelzPACTPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zPACT.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in pact.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in pact.conf");
    }
    ...
    bool showPACTAvailable = settingShowAllBalances || pactAvailableBalance != nTotalBalance;
    bool showWatchOnlyPACTAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showPACTPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyPACTPending = watchUnconfBalance != 0;
    bool showPACTLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyPACTLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showPACTAvailable || showWatchOnlyPACTAvailable);
    ui->labelBalanceText->setVisible(showPACTAvailable || showWatchOnlyPACTAvailable);
    ui->labelWatchAvailable->setVisible(showPACTAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showPACTPending || showWatchOnlyPACTPending);
    ui->labelPendingText->setVisible(showPACTPending || showWatchOnlyPACTPending);
    ui->labelWatchPending->setVisible(showPACTPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showPACTLocked || showWatchOnlyPACTLocked);
    ui->labelLockedBalanceText->setVisible(showPACTLocked || showWatchOnlyPACTLocked);
    ui->labelWatchLocked->setVisible(showPACTLocked && showWatchOnly);
    ...
    bool showzPACTAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzPACTUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzPACTImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzPACTAvailable);
    ui->labelzBalanceMatureText->setVisible(showzPACTAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzPACTUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzPACTUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzPACTImmature);
    ui->labelzBalanceImmatureText->setVisible(showzPACTImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelPACTPercent->setVisible(showPercentages);
    ui->labelzPACTPercent->setVisible(showPercentages);
