...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePACTAmount"))
        settings.setValue("nAnonymizePACTAmount", 1000);

    nAnonymizePACTAmount = settings.value("nAnonymizePACTAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePACTAmount"))
        SoftSetArg("-anonymizepactamount", settings.value("nAnonymizePACTAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pact.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePACTAmount:
            return QVariant(nAnonymizePACTAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizePACTAmount:
            nAnonymizePACTAmount = value.toInt();
            settings.setValue("nAnonymizePACTAmount", nAnonymizePACTAmount);
            emit anonymizePACTAmountChanged(nAnonymizePACTAmount);
            break;
        case CoinControlFeatures:
