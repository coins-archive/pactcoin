...
public:
    ...
    bool WriteZPACTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPACTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPACTSeed_deprecated(uint256& seed);
    bool EraseZPACTSeed();
    bool EraseZPACTSeed_deprecated();

    bool WriteZPACTCount(const uint32_t& nCount);
    bool ReadZPACTCount(uint32_t& nCount);
