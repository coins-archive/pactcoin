...
// Possible states for zPACT send
enum ZerocoinSpendStatus {
    ZPACT_SPEND_OKAY = 0,                            // No error
    ZPACT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPACT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPACT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPACT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPACT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPACT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPACT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPACT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPACT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPACT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPACT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPACT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPACT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPACT_SPENT_USED_ZPACT = 14,                      // Coin has already been spend
    ZPACT_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZPACT_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool CreateZPACTOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzpactAuto) const;
    ...
    */
    ...
    CzPACTWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzPACTTracker> zpactTracker;
    ...
    void setZWallet(CzPACTWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpactTracker = std::unique_ptr<CzPACTTracker>(new CzPACTTracker(strWalletFile));
    }

    CzPACTWallet* getZWallet() { return zwalletMain; }
